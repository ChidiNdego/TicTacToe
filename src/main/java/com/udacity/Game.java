...
public class Game {
    ...
    private char turn; // whose turn is it, 'x' or "o" ? x always starts
    ...
    public String checkGameWinner(char [][]grid){
        ...
        for (char c : player) {
            //check win on the diagonals
            if (grid[0][0] == grid[1][1] && grid[1][1] == grid[2][2] && grid[2][2] == c) {
                return (Character.toUpperCase(c) + "  wins");
            }
            if (grid[2][0] == grid[1][1] && grid[1][1] == grid[0][2] && grid[0][2] == c) {
                return (Character.toUpperCase(c) + "  wins");
            }
            ...
            for (int j = 0; j < 3; j++) {
                //rows
                if (grid[0][j] == grid[1][j] && grid[1][j] == grid[2][j] && grid[2][j] == c) {
                    return (Character.toUpperCase(c) + "  wins");
                }
                //columns
                if (grid[j][0] == grid[j][1] && grid[j][1] == grid[j][2] && grid[j][2] == c) {
                    return (Character.toUpperCase(c) + "  wins");
                }
            }
        }
